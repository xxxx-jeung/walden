...
import { postApkDownload } from '@/apis/index'

export default defineComponent({
    methods: {
        async apkDownloadClickHandler(): Promise<void> {
            const tokenPrompt = prompt('tokenKey를 입력해주세요.')

            if (tokenPrompt) {
                try {
                    const response = await postApkDownload(tokenPrompt)
